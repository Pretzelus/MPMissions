...
MON_Bld_remove = ["Bridge_PathLod_base_F","Land_Bridge_01_PathLod_F","Land_Bridge_Asphalt_PathLod_F","Land_Bridge_Concrete_PathLod_F","Land_Bridge_HighWay_PathLod_F","Land_Bridge_01_F","Land_Bridge_Asphalt_F","Land_Bridge_Concrete_F","Land_Bridge_HighWay_F"];
// ---------------------------------------------------------------------------------------------------------
...
MON_doStop = {
	...
	dostop _npc ;
	if (_sleep >= 2998) then {{_x disableAI "TARGET"} foreach units group _npc;};
	sleep _sleep;	
...
MON_moveBuildings = {
	...
	
	{
		...
		if ( _bldpos >= _minpos ) then {
			...
			if (_patrol) then {
				if (_bldpos >= 8) then {_cntobjs1 =  4};
			} else {	
				if (_bldpos >= 8) then { _cntobjs1 =   round(random 4)  + 3;};				
			};					
			...
			if (count _movein > 0) then {
				...
				if (_patrol) then {
					...
					_bldposuse = [];
					{
						...
						If ((_altura in _bldposuse)) then {
							_altura = floor(random(_bldpos));
						} else {_bldposuse = _bldposuse + [_altura];};
...
MON_CreateMine = {
	...
	if ((count _this) > 1) then {
		_mineposee = [_soldier,_position] call MON_doCreateMine;
...
//	hint format ["this: %1", _soldier];
	...
	//sleep 2;
	//_soldier fire ["MineMuzzle","MineMuzzle","ATMine"];
	//_soldier enableAI "MOVE";
