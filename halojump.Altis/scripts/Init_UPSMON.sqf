...
KRON_UPS_sharedist = 400; // org value 800 => increased for ArmA3 map sizes for less predictable missions..
...
KRON_UPS_reinforcement = true; // ToDo Set to true if UPSMON reinf is going ot be used

//Artillery support, better control if set in trigger 
KRON_UPS_ARTILLERY = false; // set to true if one of next entries is set to true !!!
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------			
	...
	UPSMON_Version = "UPSMON 6.0.2 beta1";
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if (count KRON_UPS_Player > 0) then {
		{
			_x addeventhandler ["fired",{_this spawn Aze_SN_EHfired}];	
		} foreach KRON_UPS_Player;
	};
	...
	if (isNil("KRON_UPS_INIT") || KRON_UPS_INIT == 0) then {
		...
		[] execvm "scripts\SunAngle_fnc.sqf";
